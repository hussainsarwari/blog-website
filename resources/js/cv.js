...
add_edu.addEventListener("click", () => {
    ...
    e.setAttribute(
        "class",
        "my-2 w-full h-[3em] text-sm md:text-md border-b-2 border-b-gray-500 focus:bg-blue-100   p-2 focus:outline-none text-black"
    );
...
experince_btn_add.addEventListener("click", () => {
    ...
    div_exp.setAttribute(
        "class",
        certificate_counter + " flex flex-col div_exp"
    );
...
let language_btn_add = document.querySelector(".btn_add_language");
let language_btn_remove = document.querySelector(".btn_remove_language");
let language = document.querySelector(".language");
let language_counter = 0;
let div_lan;
language_btn_add.addEventListener("click", () => {
    language_btn_remove.classList.remove("hidden");

    div_lan = document.createElement("div");
    div_lan.setAttribute("class", language_counter + " flex flex-col div_lan");

    let remove = document.createElement("input");
    remove.setAttribute("type", "checkbox");
    remove.setAttribute("class", language_counter + " checkboxs4 w-3");

    let p = document.createElement("p");
    p.textContent = "remove :   ";
    p.setAttribute("class", " text-red-400  p-2  my-2  hidden remove_flag4");

    let e = document.createElement("input");
    e.setAttribute("type", "text");
    e.setAttribute("name", "language");
    e.setAttribute("placeholder", "language");
    e.setAttribute(
        "class",
        "my-2 w-full h-[3em] text-sm md:text-md border-b-2 border-b-gray-500 focus:bg-blue-100   p-2 focus:outline-none text-black"
    );
    ...
    let e2 = document.createElement("input");
    e2.setAttribute("type", "text");
    e2.setAttribute("name", "language_write");
    e2.setAttribute("placeholder", "Writting");
    e2.setAttribute(
        "class",
        "my-2 w-full h-[3em] text-sm md:text-md border-b-2 border-b-gray-500 focus:bg-blue-100   p-2 focus:outline-none text-black"
    );
    ...
    e3.setAttribute("name", "language_speak");
    e3.setAttribute("placeholder", "Speaking");
    e3.setAttribute(
        "class",
        "my-2 w-full h-[3em] text-sm md:text-md border-b-2 border-b-gray-500 focus:bg-blue-100   p-2 focus:outline-none text-black"
    );
    ...
    e4.setAttribute("name", "language_listning");
    e4.setAttribute("placeholder", "listning");
    e4.setAttribute(
        "class",
        "my-2 w-full h-[3em] text-sm md:text-md border-b-2 border-b-gray-500 focus:bg-blue-100   p-2 focus:outline-none text-black"
    );

    div_lan.appendChild(p);
    div_lan.appendChild(e);
    div_lan.appendChild(e2);
    div_lan.appendChild(e3);
    div_lan.appendChild(e4);
    p.appendChild(remove);
    language.appendChild(div_lan);
    language_counter++;

    if (language_counter == 3) {
        language_btn_add.classList.add("hidden");
    }
});

language_btn_remove.addEventListener("click", () => {
    let re = document.querySelectorAll(".remove_flag4");
    if (language_btn_remove.textContent == "Remove") {
        re.forEach((e) => {
            e.classList.remove("hidden");
        });
        language_btn_remove.textContent = "Delete";
    } else {
        let remove_items = document.querySelectorAll(".div_lan");
        let checkboxs = document.querySelectorAll(".checkboxs4");
        re.forEach((e) => {
            e.classList.add("hidden");
        });
        language_btn_remove.textContent = "Remove";
        checkboxs.forEach((e) => {
            if (e.checked) {
                remove_items.forEach((e2) => {
                    if (e2.classList[0] == e.classList[0]) {
                        e2.remove();
                        language_btn_add.classList.remove("hidden");
                    }
                });
            }
        });
        remove_items = document.querySelectorAll(".div_lan");
        if (remove_items.length == 0) {
            language_btn_remove.classList.add("hidden");
        }
    }
});
...
let skill_btn_add = document.querySelector(".btn_add_skill");
let skill_btn_remove = document.querySelector(".btn_remove_skill");
let skill = document.querySelector(".skill");
let skill_counter = 0;
let div_skill;
skill_btn_add.addEventListener("click", () => {
    skill_btn_remove.classList.remove("hidden");

    div_skill = document.createElement("div");
    div_skill.setAttribute("class", skill_counter + " flex flex-col div_skill");

    let remove = document.createElement("input");
    remove.setAttribute("type", "checkbox");
    remove.setAttribute("class", skill_counter + " checkboxs5 w-3");

    let p = document.createElement("p");
    p.textContent = "remove :   ";
    p.setAttribute("class", " text-red-400  p-2  my-2  hidden remove_flag5");

    let e = document.createElement("input");
    e.setAttribute("type", "text");
    e.setAttribute("name", "skill");
    e.setAttribute("placeholder", "skill");
    e.setAttribute(
        "class",
        "my-2 w-full h-[3em] text-sm md:text-md border-b-2 border-b-gray-500 focus:bg-blue-100   p-2 focus:outline-none text-black"
    );
    ...
    let e2 = document.createElement("input");
    e2.setAttribute("type", "text");
    e2.setAttribute("name", "skill_write");
    e2.setAttribute("placeholder", "Writting");
    e2.setAttribute(
        "class",
        "my-2 w-full h-[3em] text-sm md:text-md border-b-2 border-b-gray-500 focus:bg-blue-100   p-2 focus:outline-none text-black"
    ...
    let e3 = document.createElement("input");
    e3.setAttribute("type", "text");
    e3.setAttribute("name", "skill_speak");
    e3.setAttribute("placeholder", "Speaking");
    e3.setAttribute(
        "class",
        "my-2 w-full h-[3em] text-sm md:text-md border-b-2 border-b-gray-500 focus:bg-blue-100   p-2 focus:outline-none text-black"
    ...
    let e4 = document.createElement("input");
    e4.setAttribute("type", "text");
    e4.setAttribute("name", "skill_listning");
    e4.setAttribute("placeholder", "listning");
    e4.setAttribute(
        "class",
        "my-2 w-full h-[3em] text-sm md:text-md border-b-2 border-b-gray-500 focus:bg-blue-100   p-2 focus:outline-none text-black"
    ...
    div_skill.appendChild(e);
    div_skill.appendChild(e2);
    div_skill.appendChild(e3);
    div_skill.appendChild(e4);
    p.appendChild(remove);
    skill.appendChild(div_skill);
    skill_counter++;

    if (skill_counter == 3) {
        skill_btn_add.classList.add("hidden");
    }
});

skill_btn_remove.addEventListener("click", () => {
    let re = document.querySelectorAll(".remove_flag5");
    if (skill_btn_remove.textContent == "Remove") {
        re.forEach((e) => {
            e.classList.remove("hidden");
        });
        skill_btn_remove.textContent = "Delete";
    } else {
        let remove_items = document.querySelectorAll(".div_skill");
        let checkboxs = document.querySelectorAll(".checkboxs5");
        re.forEach((e) => {
            e.classList.add("hidden");
        });
        skill_btn_remove.textContent = "Remove";
        checkboxs.forEach((e) => {
            if (e.checked) {
                remove_items.forEach((e2) => {
                    if (e2.classList[0] == e.classList[0]) {
                        e2.remove();
                        skill_btn_add.classList.remove("hidden");
                    }
                });
            }
        });
        remove_items = document.querySelectorAll(".div_skill");
        if (remove_items.length == 0) {
            skill_btn_remove.classList.add("hidden");
        }
    }
});
