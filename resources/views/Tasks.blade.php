...
<style>
    ...
    <div class="flex flex-row setting">
                ...
                <nav class="px-4 py-4 mt-5 lg:mt-9 lg:px-6" x-data="{ selected: $persist('Dashboard') }">
                    <div>
                        ...
                        <ul class="mb-6 flex flex-col gap-1.5">
                            ...
                            <li>
                                <a class="group active_item  relative flex items-center gap-2.5 rounded-sm px-4 py-2 font-medium text-gray-500 duration-300 ease-in-out hover:bg-graydark dark:hover:bg-meta-4"
                                    href="{{ url('profile') }}" @click="selected = (selected === 'Profile' ? '':'Profile')"
                                    :class="{ 'bg-graydark dark:bg-meta-4': (selected === 'Profile') && (page === 'profile') }"
                                    :class="page === 'profile' && 'bg-graydark'">
                                    <svg width="18" height="18" viewBox="0 0 24 24" fill="#ffffff" xmlns="http://www.w3.org/2000/svg">
                                        <path opacity="0.4" d="M11 19.5H21" stroke="#ffffff" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                                        <path opacity="0.4" d="M11 12.5H21" stroke="#ffffff" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                                        <path opacity="0.4" d="M11 5.5H21" stroke="#ffffff" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                                        <path d="M3 5.5L4 6.5L7 3.5" stroke="#ffffff" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                                        <path d="M3 12.5L4 13.5L7 10.5" stroke="#ffffff" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                                        <path d="M3 19.5L4 20.5L7 17.5" stroke="#ffffff" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                                        </svg>
                            ...
                            <li>
                                <a class="group  relative flex items-center gap-2.5 rounded-sm py-2 px-4 font-medium text-gray-500 duration-300 ease-in-out hover:bg-graydark dark:hover:bg-meta-4"
                                    href="{{ url('setting') }}">
        ...
        <main class="relative top-0 right-0 flex justify-center w-full my-3">
            <div class="w-full p-4 mx-auto md:p-6 2xl:p-10">
                    ...
                    <div class="grid grid-cols-12 gap-4 md:gap-6 2xl:gap-7.5">
                        ...
                        <div class="col-span-12">
                            <div
                                ...
                                <div>
                                    <h1 class="h-10 px-5 my-5 text-2xl text-center bg-gray-700">show task</h1>
                                    
                                    <div class="show_task">
                                        <ul class="p-5 list_tasks">
                                            <header class="grid w-full h-10 grid-cols-3 text-center ">
                                                <span class="flex justify-center align-middle ">Title</span>
                                                <span class="flex justify-center align-middle">Description</span> 
                                                <span class="flex justify-center align-middle">Status</span>
                                            </header>
                                            <hr>
                            ...
                            <div
                                ...
                                <div class="p-7">
                                  ...
                                  w-full rounded border border-stroke bg-gray py-3 pl-11.5 pr-4.5 font-medium text-black focus:border-primary focus-visible:outline-none dark:border-strokedark dark:bg-meta-4 dark:text-white dark:focus:border-primary"
                                                    ...
                                                    <label class="block my-5 text-sm font-medium text-black dark:text-white"
                                                    for="description">Description</label>
                                                    
                                                    <input
                                                        required
                                                        class="@error('description') is-invalid @enderror
                                  w-full rounded border border-stroke mt-6 bg-gray py-3 pl-11.5 pr-4.5 font-medium text-black focus:border-primary focus-visible:outline-none dark:border-strokedark dark:bg-meta-4 dark:text-white dark:focus:border-primary"
                                  ...
                                  w-full rounded border border-stroke mt-6 bg-gray py-3 pl-11.5 pr-4.5 font-medium text-black focus:border-primary focus-visible:outline-none dark:border-strokedark dark:bg-meta-4 dark:text-white dark:focus:border-primary"
                                                        ...
                                                        placeholder="Enter  task time (minutes)" />
                                                    @error('time')
                                                        <span class="text-red-500 invalid-feedback" role="alert">
                                                            <strong>{{ $message }}</strong>
                                                        </span>
                                        ...
                                        <div class="flex justify-end gap-4.5">
                                            ...
                                            <button
                                                ...
                                                type="submit">
                                                Save
                                                <h1 class="border-b-2 border-gray-500"></h1>
                                            </button>
                                        </div>
                                    </form>
                                </div>
    ...
    <script>
        ...
        window.addEventListener('load',e=>{
            let email=document.querySelector('.user_email').textContent;
            let list_task=document.querySelector('.list_tasks');
            console.log(email);
   
   fetch('http://192.168.137.1:8002/api/getTask/'+email+''
   ...
   .then(data => {
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        // li.classList.add('Tasks-item grid grid-col-3');
        li.setAttribute('class', 'grid grid-cols-3 h-[2.5em]  border-b-2 border-gray-500'); 
        list_task.appendChild(li);
        var span1 = document.createElement('span');
        span1.class='sdflk';
        span1.setAttribute('class', 'col-span-1 text-center '); 
        span1.textContent=data[i]['title'];
        li.appendChild(span1);
        

        var span2 = document.createElement('span');
        span2.setAttribute('class', 'col-span-1 text-center'); 
        span2.textContent=data[i]['description'];
        li.appendChild(span2);
       
        var span3 = document.createElement('input');
        span3.setAttribute('class', 'col-span-1 text-center h-[1.5em] '); 
        span3.setAttribute('type', 'checkbox'); 
        span3.setAttribute('disabled', 'disabled'); 
        if (data[i]['status']==1) {
        span3.setAttribute('checked', 'checked'); 
            
        }
        span3.textContent=data[i]['status'];
